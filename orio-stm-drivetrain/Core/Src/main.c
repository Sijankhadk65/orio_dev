...
int main(void)
{
  ...
  while (1)
  {
    ...
    if ((HAL_GetTick() - s_last_telemetry_poll_tick) >= WHEEL_TELEMETRY_POLL_PERIOD_MS)
    {
      s_last_telemetry_poll_tick = HAL_GetTick();
      Wheel_PollTelemetry();
    }
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(ESC_MUX_SEL_GPIO_Port, ESC_MUX_SEL_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin : ESC_MUX_SEL_Pin */
  GPIO_InitStruct.Pin = ESC_MUX_SEL_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(ESC_MUX_SEL_GPIO_Port, &GPIO_InitStruct);
